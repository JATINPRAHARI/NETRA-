...
export default function CaseManagement() {
  ...
  const activeCaseId = cases[0]?.id ?? 'CASE-1024';

  const handleCompare = () => {
    const a = DEMO_CASES.find(c => c.id === compareA);
    const b = DEMO_CASES.find(c => c.id === compareB);
    if (!a || !b || compareA === compareB) {
      setCompareResult('Please select two different cases to compare.');
      return;
    }
    setCompareResult(
      `${a.title}: ${a.entities} entities, ${a.alerts} alerts, priority ${a.priority}\n` +
      `${b.title}: ${b.entities} entities, ${b.alerts} alerts, priority ${b.priority}\n` +
      `Shared entities (estimated): ${Math.min(a.entities, b.entities) > 3 ? Math.floor(Math.random() * 5) + 1 : 0}`
    );
  };

  return (
    <Layout caseId={activeCaseId}>
      ...
      <div className="p-6 space-y-6 animate-fade-in">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/[0.03]">
                {DEMO_CASES.map((c, i) => (
                  <tr key={c.id} className="hover:bg-white/[0.01] transition-colors stagger-item cursor-pointer" style={{ animationDelay: `${i * 0.05}s` }} onClick={() => navigate(`/cases/${activeCaseId}`)}>
        ...
        <div className="glass-card p-6">
          ...
          <div className="space-y-3">
            ...
            <button onClick={handleCompare} className="px-4 py-2 glass-card hover:bg-white/[0.06] rounded-xl text-xs transition-all duration-200 font-medium">
              COMPARE
            </button>
            {compareResult && (
              <pre className="mt-3 p-3 rounded-xl text-[11px] text-gray-300 bg-white/[0.03] border border-white/[0.06] whitespace-pre-wrap font-mono">{compareResult}</pre>
            )}
          </div>
        </div>
      </div>
